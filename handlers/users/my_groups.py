...
from data.texts import profile_answer, buy_no_email_answer, select_subscription_message, \
    ...
    invalid_group_info_answer, bad_link_error_answer, invalid_group_number_answer
from logs.log_all import log_all
from states import AddGroup, DeleteGroup


@dp.message_handler(commands='my_groups')
async def command_my_groups(message: types.Message):
    try:
        ...
        if user_groups == '':
            ...
            s_group_list = ''
            for number, group in enumerate(l_group_list):
                s_group_list += f'{number+1}) {group.split(" @@ , @@ ")[0]}\n'
            await message.answer(group_list_message.format(group_list=s_group_list), reply_markup=ikb_manage_groups)
    except Exception as error:
        await message.answer(unknown_error_answer)
        await log_all('command_my_groups', 'error', message.from_user.id, message.from_user.first_name, error)
...
async def take_group_info(message: types.Message, state: FSMContext):
    try:
        ...
        if 'Bad link' in error.args:
            await message.answer(bad_link_error_answer)
            await log_all('take_group_info', 'warning', message.from_user.id, message.from_user.first_name, str(error) + '. Bad link')
            await state.finish()
            return
        await message.answer(invalid_group_info_answer)
        await log_all('take_group_info', 'warning', message.from_user.id, message.from_user.first_name, error)
        await state.finish()
    except Exception as error:
        await message.answer(unknown_error_answer)
...
async def delete_group_by_number(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(invalid_group_number_answer)
        await log_all('take_group_info', 'error', message.from_user.id, message.from_user.first_name, str(error) + ' ' + str(type(error)))
        await state.finish()
    except Exception as error:
        await message.answer(unknown_error_answer)
        await log_all('take_group_info', 'error', message.from_user.id, message.from_user.first_name, error)
